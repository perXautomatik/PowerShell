...
function Set-FileTime {
    ...
    begin {
        ...
        function touch-ExistingFile($arg) {
        ...
        function touch-NewFile([string]$path) {
            #$null > $path
    ...
    {
    function git-stree($directory = $pwd) {
...
if ( $(Test-CommandExists 'thefuck') ) {
    ...
    Set-Alias f pyhton-fuck -Option AllScope
}
}
...
if ( $IsWindows ) {
    ...
    function invoke-sublime { Start-Process "${Env:ProgramFiles}\Sublime Text\subl.exe" -ArgumentList $args -WindowStyle Hidden  } # hide subl shim script
    ...
    function invoke-bc ($REMOTE,$LOCAL,$BASE,$MERGED) { cmd /c "${Env:ProgramFiles}\BeondCompare4\BComp.exe" "$REMOTE" "$LOCAL" "$BASE" "$MERGED" } #todo: replace hardcode with global variable pointing to path
    function invoke-cygwin { param ( $cygwinpath = "${Env:ProgramFiles}\cygwin64\" ) ."$cygwinpath\Cygwin.bat" }
...
function all-processes                                    { process                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        { $_ | Select-Object * } }  # https://docs.microsoft.com/en-us/powershell/module/microsoft.powershell.core/about/about_functions?view=powershell-7#piping-objects-to-functions
function set-cdc                                    { Set-Location "$XDG_CONFIG_HOME" }
function set-cdcheat                                { Set-Location "$CHEATS_DIR" }
function set-cdd                                    { Set-Location "$DESKTOP_DIR" }
function set-cddd                                   { Set-Location "$DEVEL_DIR" }
function set-cddev                                  { Set-Location "$DEVEL_DIR" }
function set-cdn                                    { Set-Location "$NOTES_DIR" }
function set-cdports                                { Set-Location "$PORTS_DIR" }
function set-cdt                                    { Set-Location "$TODO_DIR" }
...
function export-env($name, $value)                  { set-item -force -path "env:$name" -value $value; }
function contain-folder($folder,$name)           { $q = get-childitem $folder; return $q -contains $name }
...
function invoke-grep                                   { process                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        { $_ | Select-String -Pattern $args } } # function grep($regex, $dir)                                                                                                                                                                                                                                                { if ( $dir )                                                                                                                                                                                                                                                                                                                                                                                                                                                                           { ls $dir | select-string $regex return } $input | select-string $regex }
function invoke-grepv($regex)                          { $input | ?                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     { !$_.Contains($regex) } }
...
function get-list                                   { process                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        { $_ | Format-List * } } # fl is there by default
function get-man                                    { Get-Help $args[0] | out-host -paging }
function new-dir                                { New-Item -type directory -path (Join-Path "$args" -ChildPath "") }
Set-Alias mkdir new-dir  -Option AllScope
...
function process-pgrep($name)                           { Get-Process $name }
function process-kill($name)                           { Get-Process $name -ErrorAction SilentlyContinue | kill }
function git-pull ()                                { & get pull $args }
...
function unix-sed($path, $find, $replace)            { if (!$path -or !(Test-Path $path))                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             { throw "file not found: '$path'" }  (Get-Content $path).replace("$find", $replace) | Set-Content $path }
...
function process-string                                 { process                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        { $_ | Out-String -Stream } }
function file-touch($path)                           { "" | Out-File $path -Encoding ASCII }
function properties-which($name)                           { Get-Command $name | Select-Object -ExpandProperty Definition } #should use more
